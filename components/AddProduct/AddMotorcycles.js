...
const AddMotorcycles = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const getMotorcycleBrand = async () => {
      const token = await AsyncStorage.getItem('authToken');
      try {
        const response = await fetch(`${process.env.BASE_URL}/motorcycle/brand`, {
          method: 'GET',
          headers: {
        ...
        >
          ...
          <View style={modernStyles.header}>
            ...
            <View style={modernStyles.headerContent}>
              <Text style={modernStyles.headerTitle}>{subcategory?.name || 'Motorcycle Listing'}</Text>
              <Text style={modernStyles.headerSubtitle}>Add your motorcycle details</Text>
            </View>
          </View>
